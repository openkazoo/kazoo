...
maybe_send_push_notification({Pid, ExtraHeaders}, JObj) ->
    ...
    lager:debug("pushing_dev ~s for token-id ~s : ~s"
                      ,[join_headers(Headers)
                       ,TokenID
    ...
    try
        ...
        lager:debug("apns_dev result for ~s : ~p", [Topic, Result]),
        {Code, _, Info} = Result,
...
maybe_load_apns(App, ETS) ->
    ...
    EH2 = kz_json:exec(Routines, ExtraHeaders),
...
maybe_load_apns(App, ETS, CertBin, Host, Headers) ->
    ...
    try apns:connect(Connection) of
        ...
        ?CATCH(_Er, _Ex,_ST) ->
            lager:error("error loading apns_dev ~p / ~p", [_Er, _Ex]),
            ?LOGSTACK(_ST),
            ...
            case catch apns:connect(Connection) of
                {'ok', Pid} ->
                    ...
                    {Pid, Headers};
                {'error', Reason} ->
                    lager:error("error loading2 apns_dev ~p", [Reason]),
                    'undefined'
            end
    end.
...
%% apns_expiration should be This value is a UNIX epoch expressed in seconds (UTC).
