...
compact_design_docs(_Compactor, _Shard, []) ->
    'ok';
compact_design_docs(Compactor, Shard, DDs) ->
    ...
    try lists:split(?MAX_COMPACTING_VIEWS, DDs) of
        {Compact, Remaining} ->
            IsStarted = [
                {DD, kz_couch_view:design_compact(Conn, Shard, DD)}
             || DD <- Compact
            ],
        ...
        'error':'badarg' ->
            ...
            lager:debug("  is shard ~s compacting last chunk of views started: ~p", [
                Shard, IsStarted
            ]),
...
-type db_info_resp() ::
    {'ok', kz_json:object()}
    | {'error', any()}.
-type design_info_resp() ::
    {'ok', kz_json:object()}
    | {'error', any()}.

-spec wait_for_design_compaction(kz_data:connection(), kz_term:ne_binary(), kz_term:ne_binaries()) ->
    'ok'.
wait_for_design_compaction(_, _, []) ->
    'ok';
wait_for_design_compaction(AdminConn, Shard, [DD | DDs]) ->
    wait_for_design_compaction(
        AdminConn, Shard, DDs, DD, kz_couch_view:design_info(AdminConn, Shard, DD)
    ).

-spec wait_for_design_compaction(
    kz_data:connection(),
    kz_term:ne_binary(),
    kz_term:ne_binaries(),
    kz_term:ne_binary(),
    ...
    'ok'.
wait_for_design_compaction(
    AdminConn, Shard, DDs, DD, {'error', {'conn_failed', {'error', 'timeout'}}}
) ->
...
wait_for_compaction(AdminConn, Shard, {'ok', ShardData}) ->
    case kz_json:is_true(<<"compact_running">>, ShardData, 'false') of
        'false' ->
            lager:debug("compaction has ended");
        'true' ->
            'ok' = timer:sleep(?SLEEP_BETWEEN_POLL),
            wait_for_compaction(AdminConn, Shard)
    end.
...
should_compact(_Compactor, ?HEUR_NONE) ->
    'true';
should_compact(Compactor, ?HEUR_RATIO) ->
    case get_db_disk_and_data(compactor_conn(Compactor), compactor_database(Compactor)) of
        ...
        'not_found' ->
            lager:debug(
                "db ~s not found on node ~s, skipping",
                [compactor_database(Compactor), compactor_node(Compactor)]
            ),
            'false'
    end.
...
get_db_disk_and_data(Conn, DbName, N) ->
    N > 0 andalso
        lager:debug("getting db info re-attempt ~p", [N]),
...
should_compact_ratio(Disk, Data) ->
    min_data_met(Data, ?MIN_DATA) andalso
        min_ratio_met(Disk, Data, ?MIN_RATIO).
...
min_ratio_met(Disk, Data, MinRatio) ->
    case (Disk - Data) / Disk * 100 of
        R when R > MinRatio ->
            lager:debug(
                "ratio ~.2f% (~p disk/~p data) is greater than min ratio: ~p%",
                [R, Disk, Data, MinRatio]
            ),
            'true';
        _R ->
            lager:debug(
                "ratio ~.2f% ((~p-~p) / ~p * 100) is under min threshold ~p%",
...
-spec node_shards(kz_term:ne_binary(), kz_term:ne_binary(), kz_json:object()) ->
    kz_term:ne_binaries().
node_shards(Node, Unencoded, DbInfo) ->
