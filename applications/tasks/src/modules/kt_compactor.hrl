...
-define(MIN_DATA,
    %% 128Kb
    kapps_config:get_integer(?SYSCONFIG_COUCH, <<"min_data_size">>, 131072)
).
-define(MAX_WAIT_FOR_COMPACTION_PIDS,
    case
        kapps_config:get(
            ?SYSCONFIG_COUCH, <<"max_wait_for_compaction_pids">>, 360 * ?MILLISECONDS_IN_SECOND
        )
    of
        <<"infinity">> -> 'infinity';
...
-define(SLEEP_BETWEEN_POLL,
    kapps_config:get_integer(
        ?SYSCONFIG_COUCH, <<"sleep_between_poll">>, 3 * ?MILLISECONDS_IN_SECOND
    )
).
