...
build_load_params(Context, View, Options) ->
    try build_general_load_params(Context, View, Options) of
        #{direction := Direction} = LoadMap ->
            HasQSFilter =
                not props:get_is_true('no_filter', Options, 'false') andalso
...
build_load_range_params(Context, View, Options) ->
    try build_general_load_params(Context, View, Options) of
        #{direction := Direction} = LoadMap ->
            ...
            HasQSFilter =
                not props:get_is_true('no_filter', Options, 'false') andalso
                    crossbar_filter:is_defined(Context) andalso
                    not crossbar_filter:is_only_time_filter(Context, TimeFilterKey),

            lager:debug("has qs filter: ~s", [HasQSFilter]),

            case time_range(Context, Options, TimeFilterKey) of
                {StartTime, EndTime} ->
                    StartEnd = ranged_start_end_keys(
                        Context, Options, Direction, StartTime, EndTime
                    ),
                    KeyMinLength = props:get_value('key_min_length', Options),
                    {StartKey, EndKey} = expand_min_max_keys(StartEnd, Direction, KeyMinLength),
                    Params = LoadMap#{
                        end_time => EndTime,
                        has_qs_filter => HasQSFilter,
                        mapper => crossbar_filter:build_with_mapper(
                            Context, UserMapper, HasQSFilter
                        ),
                        start_key => StartKey,
                        start_time => StartTime,
                        view_options => build_view_query(
                            Options, Direction, StartKey, EndKey, HasQSFilter
                        )
                    },
                    maybe_set_start_end_keys(Params, StartKey, EndKey);
                Ctx ->
