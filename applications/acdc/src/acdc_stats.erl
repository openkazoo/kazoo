...
-spec call_processed(
    kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), atom()
) -> 'ok'.
...
cleanup_data(Srv) ->
    ...
    StatusMatch = [
        {#status_stat{key = #status_stat_key{timestamp = '$1'}, _ = '_'}, [{'=<', '$1', Past}], [
            '$_'
        ]}
    ],
...
handle_missed_stat(JObj, Props) ->
    ...
    case find_call_stat(Id) of
        'undefined' ->
            lager:debug("can't update stat ~s with missed data, missing", [Id]);
