...
-spec agent_paused(
    kz_term:ne_binary(), kz_term:ne_binary(), timeout() | 'undefined', kz_term:api_binary()
) -> 'ok'.
...
handle_status_stat(JObj, Props) ->
    ...
    gen_listener:cast(
        ...
        {'create_status', #status_stat{
            key = status_stat_key(AccountId, AgentId, Timestamp),
...
query_statuses(RespQ, MsgId, Match, Limit) ->
    ...
    Resp = [
        {<<"Agents">>, query_statuses_group_by_agent(Stats, Limit)},
        {<<"Msg-ID">>, MsgId}
        | kz_api:default_headers(?APP_NAME, ?APP_VERSION)
    ],
...
query_statuses_fold([], _, StatsByAgent) ->
    kz_json:from_map(StatsByAgent);
query_statuses_fold(
    [
        #status_stat{
            key = #status_stat_key{
    ...
    StatsByAgent1 =
        case
            Limit =:= 'no_limit' orelse
                maps:size(AgentStats) < Limit
        of
            'true' ->
...
status_stat_to_map(#status_stat{
    key = #status_stat_key{
        agent_id = AgentId,
        timestamp = Timestamp
    },
...
archive_status_data(Srv, 'false') ->
    ...
    Match = [
        {
            #status_stat{
                key = #status_stat_key{timestamp = '$1'},
                is_archived = '$2',
                _ = '_'
            },
            [
...
maybe_archive_status_data(Srv, Match) ->
    case ets:select(status_table_id(), Match) of
        ...
        Stats ->
            ...
            _ = [
                gen_listener:cast(
                    Srv, {'update_status', Id, Key, [{#status_stat.is_archived, 'true'}]}
                )
