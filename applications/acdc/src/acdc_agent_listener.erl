...
-record(state, {
    ...
    % the ACDc Queue ID
    acdc_queue_id :: kz_term:api_ne_binary(),
    % the AMQP Queue ID of the ACDc Queue process
    msg_queue_id :: kz_term:api_ne_binary(),
...
-define(BINDINGS(AcctId, AgentId), [
    {'self', []},
    {'acdc_agent', [
        {'account_id', AcctId},
        {'agent_id', AgentId},
...
-define(RESPONDERS, [
    ...
    {{'acdc_agent_handler', 'handle_queue_started_notif'}, [{<<"queue">>, <<"started_notif">>}]}
]).
...
refresh_config(Srv, Qs, StateName) -> gen_listener:cast(Srv, {'refresh_config', Qs, StateName}).
...
terminate(Reason, #state{
    ...
    Reason =:= 'normal' andalso
        kz_util:spawn(fun acdc_agents_sup:stop_agent/2, [AcctId, AgentId]),
...
do_send_availability_update(QueueId, 'ready', #state{
    agent_id = AgentId,
    acct_id = AcctId
}) ->
    send_agent_available(AcctId, AgentId, QueueId);
do_send_availability_update(QueueId, _, #state{
    agent_id = AgentId,
    acct_id = AcctId
}) ->
