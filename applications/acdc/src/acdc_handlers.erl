...
maybe_route_respond(ReqJObj, Call, AcctId, Id) ->
    case kz_datamgr:open_cache_doc(kapps_call:account_db(Call), Id) of
        {'error', _} -> 'ok';
        {'ok', Doc} -> maybe_route_respond(ReqJObj, Call, AcctId, Id, kz_doc:type(Doc))
    end.
...
update_acdc_actor(Call, AgentId, <<"user">>) ->
    ...
    case acdc_agent_util:most_recent_status(AcctId, AgentId) of
        {'ok', <<"logged_out">>} ->
            update_acdc_agent(
                Call, AcctId, AgentId, <<"login">>, fun kapi_acdc_agent:publish_login/1
            );
        {'ok', <<"pause">>} ->
            update_acdc_agent(
                Call, AcctId, AgentId, <<"resume">>, fun kapi_acdc_agent:publish_resume/1
            );
        {'ok', _S} ->
            update_acdc_agent(
                Call, AcctId, AgentId, <<"logout">>, fun kapi_acdc_agent:publish_logout/1
            )
    end.
