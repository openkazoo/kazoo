...
has_agents(Srv, EnterWhenEmpty) ->
    EnterWhenEmpty orelse
        gen_listener:call(Srv, 'get_has_agents').
...
start_queue_call(JObj, Props, Call) ->
    ...
    JObj2 = kz_json:set_value(
        [<<"Call">>, <<"Custom-Channel-Vars">>, <<"Queue-ID">>], QueueId, JObj
    ),
...
-spec should_ignore_member_call(kz_types:server_ref(), kapps_call:call(), kz_json:object()) ->
    boolean().
...
-spec should_ignore_member_call(
    kz_types:server_ref(), kapps_call:call(), kz_term:ne_binary(), kz_term:ne_binary()
) -> boolean().
...
-spec handle_call(any(), kz_term:pid_ref(), mgr_state()) ->
    kz_types:handle_call_ret_state(mgr_state()).
handle_call(
...
handle_call(
    ...
    case queue:out(Agents) of
        {{'value', Winner}, Agents1} ->
            ...
            {'reply', Winner,
                State#state{strategy_state = SS#strategy_state{agents = queue:in(Winner, Agents1)}},
                'hibernate'};
        {'empty', _} ->
            {'reply', 'undefined', State}
    end;
...
handle_cast({'monitor_call', Call}, State) ->
    ...
    gen_listener:add_binding(self(), 'call', [
        {'callid', CallId},
...
handle_cast(
    ...
    {'noreply', State};
handle_cast({'agent_available', AgentId}, #state{supervisor = QueueSup} = State) when
    is_binary(AgentId)
->
    Data = #{},
...
handle_cast({'agent_ringing', AgentId}, #state{strategy = Strategy} = State) when
    is_binary(AgentId)
->
...
handle_cast({'agent_unavailable', AgentId}, #state{strategy = Strategy} = State) when
    is_binary(AgentId)
->
...
handle_cast(
    ...
    {'noreply', State};
handle_cast(
    {'gen_listener', {'created_queue', ?SECONDARY_QUEUE_NAME(QueueId)}},
    #state{queue_id = QueueId} = State
) ->
    {'noreply', State};
handle_cast(
    {'gen_listener', {'created_queue', _}},
    #state{
        account_id = AccountId,
        queue_id = QueueId
    } = State
...
handle_cast(
    ...
    Call = kapps_call:set_custom_channel_var(
        <<"Queue-Position">>,
        Position,
    ...
    {'noreply', State#state{
        current_member_calls = [Call | Calls],
...
handle_cast({'handle_queue_member_add', JObj}, #state{current_member_calls = CurrentCalls} = State) ->
    ...
    {'noreply', State#state{
        current_member_calls = [Call | lists:keydelete(CallId, 2, CurrentCalls)]
    }};
...
-spec publish_member_call_failure(
    kz_term:ne_binary(),
    kz_term:ne_binary(),
    kz_term:ne_binary(),
    kz_term:ne_binary(),
...
%% @end
%%------------------------------------------------------------------------------
-spec update_strategy_with_agent(mgr_state(), kz_term:ne_binary(), agent_change()) ->
    strategy_state().
...
-spec update_strategy_with_agent(
    mgr_state(), kz_term:ne_binary(), agent_change(), agent_change_data()
) ->
    strategy_state().
update_strategy_with_agent(
    ...
    maybe_send_diagnostics_for_strategy_update(
        Strategy, Agents, AgentId, RingingAgents, BusyAgents
    ),
    SS2.
...
%% @end
%%------------------------------------------------------------------------------
-spec do_update_strategy_with_agent(
    mgr_state(), kz_term:ne_binary(), agent_change(), agent_change_data()
) ->
    strategy_state().
do_update_strategy_with_agent(
    #state{
        strategy = 'rr',
        strategy_state = SS
    },
...
-spec update_rr_strategy_with_agent(
    strategy_state(), kz_term:ne_binary(), agent_change(), agent_change_data()
) ->
    strategy_state().
update_rr_strategy_with_agent(
    #strategy_state{agents = AgentQueue} = SS,
    AgentId,
    'available',
    _
) ->
...
update_mi_strategy_with_agent(#strategy_state{agents = AgentL} = SS, AgentId, 'available') ->
    lists:member(AgentId, AgentL) orelse
...
update_mi_strategy_with_agent(#strategy_state{agents = AgentL} = SS, AgentId, _) ->
    lists:member(AgentId, AgentL) andalso
...
-spec cancel_position_announcements(kapps_call:call() | 'false', map()) ->
    map().
cancel_position_announcements('false', Pids) ->
    Pids;
...
remove_queue_member(
    ...
    AnnouncementsPids1 = cancel_position_announcements(
        lists:keyfind(CallId, 2, CurrentCalls), AnnouncementsPids
    ),

    State#state{
        current_member_calls = lists:keydelete(CallId, 2, CurrentCalls),
        announcements_pids = AnnouncementsPids1
    }.
...
%% @end
%%------------------------------------------------------------------------------
-spec maybe_send_diagnostics_for_strategy_update(
    queue_strategy(),
