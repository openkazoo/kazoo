...
most_recent_ets_status(AccountId, AgentId) ->
    case most_recent_ets_statuses(AccountId, AgentId) of
        {'error', _} = E -> E;
        {'ok', Statuses} -> most_recent_ets_agent_status(kz_json:get_json_value(AgentId, Statuses))
    end.

-spec most_recent_ets_agent_status(kz_term:api_object()) ->
    {'ok', kz_term:ne_binary()}
    | {'error', 'not_found'}.
most_recent_ets_agent_status('undefined') ->
    {'error', 'not_found'};
...
most_recent_statuses(AccountId, AgentId, Options) ->
    ...
    DBStatuses =
        case fetch_db_statuses(AccountId, AgentId) of
...
filter_agent_statuses(Statuses, 'undefined') ->
    Statuses;
