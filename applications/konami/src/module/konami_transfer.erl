    ...
    try
        gen_statem:enter_loop(
            ...
            #state{
                ...
                takeback_dtmf = kz_json:get_value(
                    <<"takeback_dtmf">>, Data, ?DEFAULT_TAKEBACK_DTMF
                ),
                ringback = to_tonestream(
                    kz_json:get_value(<<"ringback">>, Data, ?DEFAULT_RINGBACK)
                ),
                moh = find_moh(Data, Call),
                extension = get_extension(
                    kz_json:get_first_defined([<<"captures">>, <<"target">>], Data)
                )
            }
        )
    of
