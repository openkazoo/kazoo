...
validate_schema(<<Schema/binary>>, {Doc, ValidationErrors}, OnSuccess) ->
    ...
    case kz_json_schema:load(Schema) of
        ...
        {'error', 'not_found'} when SchemaRequired ->
            ...
            throw(
                {'system_error', {'datastore_fault', <<"schema '", Schema/binary, "' not found.">>}}
            );
        {'error', 'not_found'} ->
            lager:error(
                "~s schema not found, assuming schema validation passed, continuing anyway", [
                    Schema
                ]
            ),
...
validate_schema(SchemaJObj, {Doc, ValidationErrors}, OnSuccess) ->
    ...
    try kz_json_schema:validate(SchemaJObj, kz_doc:public_fields(Doc), Options) of
        ...
        {'error', SchemaErrors} ->
            lager:debug(
                "schema validation errors but not strictly validating, trying to fix request"
            ),
        ...
        ?STACKTRACE('error', 'function_clause', ST)
            ...
            throw(
                {'system_error',
                    {'function_clause',
                        <<"function clause failure - schema validation failed to run on the server">>}}
            )
    end.
